...
module.exports = {
	createBuffer(data) {
		...
		if (dataTypeName === 'string') {
			// String
			const strLen = data.length + 1;
			sizeBuf = Buffer.alloc(1);
			sizeBuf.writeUInt8(strLen);
			buffer = Buffer.from(data, 'utf8');
			buffer = Buffer.concat([sizeBuf, buffer, endStringBuffer], buffer.length+2);
	...
	parse: function(buffer, index) {
		...
		switch(type) {
			...
			case "f16":
			case "f32":
				parseValue.floatle("val");
				break;
			case "f64":
				parseValue.doublele("val");
				break;
			case "string":
				var parseSize = new Parser().skip(index+1).uint8("length");
				const strLen = parseSize.parse(buffer).length-1;
				parseValue.skip(1).string("val", {length: strLen, zeroTerminated: true});
				break;
			case "buffer":
				var parseSize = new Parser().skip(index+1).uint8("length");
				const arrLen = parseSize.parse(buffer).length;
				parseValue.skip(1).buffer("val", {length: arrLen});
				break;
		}

	...
	parse: function(buffer, index) {
		...
		switch(type) {
			...
			case "f64":
				...
				break;
			case "string":
				parseValue.uint8("length");
				const strLen = parseValue.parse(buffer).length;
				parseValue.string("val", {length: strLen, zeroTerminated: true});
				break;
			case "buffer":
				parseValue.uint8("length");
				const arrLen = parseValue.parse(buffer).length;
				parseValue.buffer("val", {length: arrLen});
				break;
		}
